...
heat_map = [[int(n) for n in l] for l in open("AdventOfCode/2023/examples/day17.in").read().strip().split("\n")]
...
class HeatMazeAStar(AStar):
    ...
    def neighbors(self, node):
        n, d = node
        ns = [self.walk(n, d, 1), self.walk(n, d, -1), self.walk(n, d, 1j), self.walk(n, d, -1j)]
...
print("Part 1:", sum(heat(heat_map, list(HeatMazeAStar(heat_map).astar((0, 0), ((len(heat_map) - 1) + (len(heat_map)-1) * 1j, 0))))))
...
print("Part 2:", sum(heat(heat_map, list(HeatMazeAStarP2(heat_map).astar((0, 0), ((len(heat_map) - 1) + (len(heat_map)-1) * 1j, 0))))))
